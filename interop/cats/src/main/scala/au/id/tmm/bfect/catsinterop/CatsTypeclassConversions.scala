...
trait CatsToBfectTypeclassConversions {
  ...
  implicit def catsBifunctorIsBfectBifunctor[F[_, _]](implicit catsBifunctor: cats.Bifunctor[F]): Bifunctor[F] =
    new Bifunctor[F] {
      override def biMap[L1, R1, L2, R2](f: F[L1, R1])(leftF: L1 => L2, rightF: R1 => R2): F[L2, R2] =
        catsBifunctor.bimap(f)(leftF, rightF)
