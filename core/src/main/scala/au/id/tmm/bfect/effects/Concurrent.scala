  ...
  trait WithBMonad[F[_, _]] extends Concurrent[F] { self: BMonad[F] =>
    override def race[E, A, B](fea: F[E, A], feb: F[E, B]): F[E, Either[A, B]] =
      ...
      flatMap(racePair(fea, feb)) {
        case Left((a, fibreForB))  => map[E, Unit, Either[A, B]](leftWiden(fibreForB.cancel))(_ => Left(a))
        case Right((fibreForA, b)) => map[E, Unit, Either[A, B]](leftWiden(fibreForA.cancel))(_ => Right(b))
      }
    /*_*/
