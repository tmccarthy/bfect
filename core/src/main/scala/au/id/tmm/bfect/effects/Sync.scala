...
trait Sync[F[_, _]] extends Die[F] {
  ...
  def sync[A](block: => A): F[Nothing, A] = suspend[Nothing, A](rightPure[Nothing, A](block))
...
object Sync extends SyncStaticOps {
  ...
  trait ToSyncOps {
    implicit def toSyncOps[F[_, _], E, A](fea: F[E, A])(implicit timerInstance: Sync[F]): Ops[F, E, A] =
      new Ops[F, E, A](fea)

    implicit def toSyncOpsErrorNothing[F[_, _], A](
      fea: F[Nothing, A],
    )(implicit
      timerInstance: Sync[F],
    ): Ops[F, Nothing, A] =
      new Ops[F, Nothing, A](fea)

    implicit def toSyncOpsValueNothing[F[_, _], E](
      fea: F[E, Nothing],
    )(implicit
      timerInstance: Sync[F],
    ): Ops[F, E, Nothing] =
    ...
    implicit def toSyncOpsErrorNothingValueNothing[F[_, _]](
  ...
  final class Ops[F[_, _], E, A](fea: F[E, A])(implicit sync: Sync[F])
...
trait SyncStaticOps extends DieStaticOps {
  ...
  def bracketCloseable[F[_, _] : Sync : Bracket, R, E, E2 >: E, A](
    ...
    Bracket[F].bracket[R, E2, A](Sync[F].leftWiden(acquire), r => Sync[F].sync(r.close()), use)
